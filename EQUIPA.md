# Equipa e escalas

Tirado das mensagens da cliente.

Para fora, ninguém aparece com o nome verdadeiro: cada uma é
`Profissional 1`, `2`, `3`, `4`. O nome real só existe dentro da gestão.

## Valongo

---

## Profissional 1 — Ariadna

Cabeleireira unisexo.

| Dia | Entrada | Saída |
|---|---|---|
| Terça | 09:00 | 21:00 |
| Quarta | 09:00 | 21:00 |
| Quinta | 09:00 | 21:00 |
| Sexta | 09:00 | 21:00 |
| Sábado | 09:00 | 21:00 |

Segunda e domingo: folga.

## Profissional 2 — Adyr

Cabeleireira unisexo.

| Dia | Entrada | Saída |
|---|---|---|
| Segunda | 14:00 | 21:00 |
| Quarta | 10:00 | 21:00 |
| Quinta | 14:00 | 21:00 |
| Sexta | 10:00 | 21:00 |
| Sábado | 10:00 | 21:00 |

Terça: folga. **Domingos alternados** — a mensagem diz "alguns domingos
alternados", sem dizer quais. Isto não se consegue escalar sozinho; ou
se define a regra (um sim, um não? de quinze em quinze dias?) ou ela
abre esses domingos à mão, um a um, na gestão.

## Profissional 3 — Nana 💅

Unhas.

| Dia | Entrada | Saída |
|---|---|---|
| Segunda | 10:00 | 18:00 |
| Terça | 10:00 | 18:00 |
| Quinta | 09:00 | 17:00 |

Quarta, sexta, sábado e domingo: folga.

## Profissional 4 — Filipa

| Dia | Entrada | Saída |
|---|---|---|
| Segunda | 09:00 | 18:00 |
| Terça | 09:00 | 15:00 |
| Quarta | 09:00 | 18:00 |
| Sexta | 09:00 | 21:00 |
| Sábado | 09:00 | 21:00 |

Quinta e domingo: folga.

---

## Maia

## Profissional 5 — a dona

Cabeleireira unisexo. **Está sozinha na Maia.**

| Dia | Entrada | Saida |
|---|---|---|
| ? | 09:00 | 20:00 |

As horas sao estas todos os dias que abre. **Que dias sao, nao foi
dito.** Ela disse que depois confirma mais pessoas para esta loja.

---

## Por confirmar

1. **A linha solta dos domingos.** A mensagem diz *"Os Domingos e das
   9:00h até as 21:00h"* entre o bloco da Nana e o da Filipa. Não dá
   para saber de quem é. As duas leituras possíveis: ou é o horário da
   loja ao domingo, ou são os domingos alternados da Adyr. Muda a escala
   de quem, portanto fica por decidir.

2. **Que dias abre a Maia.** Sabe-se a hora (09:00-20:00) mas nao os
   dias. Sem isso a Maia nao aceita marcacoes.

3. **Que serviços faz cada uma.** Sabe-se que a Ariadna e a Adyr são
   cabeleireiras e que a Nana faz unhas. Da Filipa não se sabe. E é isto
   que decide o que aparece a cada uma no funil: uma cliente que escolhe
   "Manicure normal" só pode cair na Nana.

4. **Quem manda na loja.** Nenhuma delas foi indicada como gerente. Por
   enquanto todas entram como profissionais — vêem só a agenda delas.

5. **Telemóvel de cada uma.** É com o telemóvel que se entra no sistema,
   não com o email. Sem ele não há conta.
